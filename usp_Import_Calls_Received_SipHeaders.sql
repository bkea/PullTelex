...
/****** Object:  StoredProcedure [Import].[usp_Import_Calls_Received_SipHeaders]    Script Date: 9/16/2021 4:24:39 PM ******/
SET ANSI_NULLS ON
GO
...
IF OBJECT_ID('Import.usp_Import_Calls_Received_SipHeaders', 'P') IS NOT NULL
    DROP PROCEDURE Import.usp_Import_Calls_Received_SipHeaders
GO

CREATE PROCEDURE [Import].[usp_Import_Calls_Received_SipHeaders]
(
	@json NVARCHAR(MAX)
)
AS
...
-- Description:	Imports JSON documents into Import_Calls_Received_SipHeaders table
-- =============================================
BEGIN
/*
...
BEGIN TRY
	...
	WHERE 
		SipHeaders_HeaderName IS NOT NULL
		AND SipHeaders_HeaderValue IS NOT NULL
	
	INSERT INTO [Import].[CallsReceived_SipHeaders]
		(
		IvrId,
		GeneralInfo_CallId,
		SipHeaders_HeaderName,
		...
		SELECT
			...
			SipHeaders_HeaderValue
		FROM
			[Import].[CallsReceived_SipHeaders]
