...
{
    public class GetAgentStatus : IGetAgentStatus
    {
        private readonly IHttpClientFactory _httpClientFactory;
        private readonly ILogger<GetQueues> _logger;
        ...
        private const string ApiUrl = "https://pop1-apps.mycontactcenter.net/api/v2/hist/agentstatus/";

        public GetAgentStatus(IHttpClientFactory httpClientFactory, ILogger<GetQueues> logger, ISharedLibs shared)
        {
        ...
        public async Task<string> GetAgentStatusAsync()
        {
            try
            {
                ...
                for (int i = 1; i <= loop_counter; i++)
                {
                    ...
                    rowsinserted = callRepo.InsertRows(jsonDocs, "Import.usp_Import_AgentStatus");
                    rowsaffected += rowsinserted;
                    _httpClient.DefaultRequestHeaders.Remove("page");
                }

                return jsonDocs is null
                    ? $"No datat Pulled for Status "
                    : $"Agent Status: {rowsaffected} rows inserted";
            }
            catch (HttpRequestException httpEX)
            {
                _logger.LogError(httpEX.Message);
                return $"Status Inserts failed. Http Status Code {httpEX.StatusCode}";
            }
            catch (Exception ex)
            {
                _logger.LogError(ex.Message);
                return $"Status Inserts failed! {ex}";
            }
        }
    }
}